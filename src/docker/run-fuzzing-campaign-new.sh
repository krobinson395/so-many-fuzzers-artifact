...
then
       ...
       corpus_option="IS_CORPUS=1"
       echo "with corpus option"
fi
...
function run_trial {
  ...
  mkdir "${shared_folder}"
  cp -r ${input_sync}/log ${shared_folder}
  ...
  cid=$(docker run -dt --rm  --security-opt seccomp:unconfined \
    ...
    bash -c "source ${WORKDIR_PATH}/.bashrc \
     && cd script \
     && ${corpus_option} ./validate-new.sh ${TIMEOUT}")
...
echo "[+] Run new validation oracle:"
echo "    - validating ${input_sync}"
echo "    - in ${TAG}"
echo "    - at $(date -u '+%F %R')"
